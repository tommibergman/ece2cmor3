...
def execute(tasks):
    ...
    for k,v in taskdict.iteritems():
        ...
        for task,executed in taskmask.iteritems():
            if(not executed):
                log.error("The source variable %s could not be found in the input tm5 data" % task.source.var_id)
...
def execute_netcdf_task(task,tableid):
    ...
    cmor_utils.netcdf2cmor(varid, ncvar, 0, factor, store_var, get_ps_var(getattr(ps_tasks[task.target.frequency],cmor_task.output_path_key,None)),
